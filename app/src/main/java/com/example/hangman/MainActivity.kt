    ...
    fun check(view: View) {
        ...
        if (!letters.contains(let)) {
            ...
            if (count != 0) { // letter is in word
                ...
                Toast.makeText(this, "Źle! Próbuj dalej!", Toast.LENGTH_SHORT).show()
                // change image, check if not lost
                lost++
                binding.image.setImageResource(images[lost])
                if (lost == images.size - 1) {
                    Toast.makeText(this, "Przegrywasz! Hasło: " + dictionary[index], Toast.LENGTH_SHORT).show()
                    initializeGame()
                }
            }
        } else {
            Toast.makeText(this, "Ta litera już była...", Toast.LENGTH_SHORT).show()
        }
    }
}